...
//! Emit the JSON Schema for the `kind: Maintenance` YAML document to stdout.
//!
//! Run via `make schema` (which redirects into
//! `schemas/maintenance.schema.json`). The committed schema is the editor-facing
//! artifact; the drift-check test fails CI if the model moves ahead of it.
