...
//! Emit the JSON Schema for the composite `kind: Requisition` YAML
//! document to stdout.
//!
//! Run via `make schema` (which redirects into
//! `schemas/requisition.schema.json`). The committed schema is the
//! editor-facing artifact; the drift-check tests in
//! `tests/schema_drift.rs` fail CI if the type definitions or the
//! annotation table move ahead of the committed file.
//!
//! After the base schema is generated from the `RequisitionLocal`
//! derive, this binary post-processes it to inject the project's
//! `x-onmsctl-list-kind` extension on each list-shaped field per the
//! single-source-of-truth table in [`onmsctl_provisioning::schema::ANNOTATIONS`].
//! `set`-kind entries also gain `"uniqueItems": true` so editors catch
//! duplicates at edit-time alongside the diff-engine dedup at apply-time.

use onmsctl_provisioning::model::RequisitionLocal;
use onmsctl_provisioning::schema::ANNOTATIONS;
use serde_json::Value;

fn main() {
    let schema = schemars::schema_for!(RequisitionLocal);
