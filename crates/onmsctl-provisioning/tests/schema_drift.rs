...
//! Drift check between the committed `schemas/requisition.schema.json`
//! artifact and the schema regenerated from `RequisitionLocal` (plus
//! the `x-onmsctl-list-kind` / `uniqueItems` post-processing in
...
//!
//! The annotation table lives in `onmsctl_provisioning::schema::ANNOTATIONS`
...
fn committed_schema_matches_generated() {
    ...
    if committed != generated {
        panic!(
            "schemas/requisition.schema.json is stale — run `make schema` and commit the result.\n\
             Tip: `diff <(cat schemas/requisition.schema.json) <(cargo run --quiet --release --example gen_schema -p onmsctl-provisioning)`"
        );
    }
}
...
fn list_kind_annotations_match_table_exactly() {
    ...
    collect_annotated_paths(&json, &mut Vec::new(), &mut found);
