...
//! Drift check between the committed `schemas/business-service.schema.json`
//! artifact and the schema regenerated from `BusinessServiceLocal` at test time.
...
fn committed_schema_matches_generated() {
    ...
    if committed != generated {
        panic!(
            "schemas/business-service.schema.json is stale — run `make schema` and commit the result.\n\
             Tip: `diff <(cat schemas/business-service.schema.json) <(cargo run --quiet --release --example gen_schema -p onmsctl-businessservice)`"
        );
    }
}
...
fn committed_schema_path() -> std::path::PathBuf {
    ...
    std::path::Path::new(manifest_dir)
        ...
        .join("..")
        .join("schemas")
        .join("business-service.schema.json")
}
