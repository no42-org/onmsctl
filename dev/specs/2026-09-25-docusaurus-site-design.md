...
```
docs/
├── intro.md                     README pitch, what it does, kubectl model
├── getting-started/
...
- New source files in `website/` (`docusaurus.config.ts`, `sidebars.ts`, `src/css/custom.css`) carry the SPDX Apache-2.0 header. Markdown does not.
...
- No custom React pages. `intro.md` is the landing page.
- Footer link columns:
...
- New `docs.yml` deploys the site:
  - Triggers: push to `main` touching `docs/**`, `website/**` or `examples/**`, plus `workflow_dispatch`.
