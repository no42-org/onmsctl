...
For Oh My Zsh, write to `~/.oh-my-zsh/custom/completions/_onmsctl` and add `fpath=("$ZSH_CUSTOM/completions" $fpath)` to `~/.zshrc` above the `source $ZSH/oh-my-zsh.sh` line.
